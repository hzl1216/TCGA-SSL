...
def tcga(data, annotation_path):
    ...
    features = np.array(big_table.iloc[:, 1:])
    # features[np.where(features <= 1)] = 0
    #
    # features = np.log2(1.0 + features)
    ...
    np.savetxt('feature_name_tcga.txt', feature_name,delimiter=',',fmt = '%s')

    kf= KFold(n_splits=10, shuffle=True, random_state=13)
    for index, (train_index, test_index) in enumerate(kf.split(features)):
        x_train, x_test, y_train, y_test = features[train_index], [test_index], labels[train_index], labels[test_index]
        scaler = normalise_and_save(x_train, feature_name, y_train , file_path='data/train_tcga_%d.csv'%index)
        normalise_and_save(x_test, feature_name, y_test,scaler, 'data/test_tcga_%d.csv'%index)
...
def merge_geo_tcga(tcga_data, geo_data, annotation_path):
    ...
    feature_name = np.array(feature_name)[idx2]
    np.savetxt('feature_name_merge.txt', feature_name,delimiter=',',fmt = '%s')
    kf = KFold(n_splits=10, shuffle=True, random_state=13)
    ...
    for index, (train_index, test_index) in enumerate(kf.split(features_t)):
        ...
        normalise_and_save(x_train, feature_name, y_train, scaler, 'data/train_merge_%d.csv'%index)
        normalise_and_save(x_test, feature_name, y_test, scaler, 'data/test_merge_%d.csv'%index)
...
#    tcga('data/tcga.csv', 'data/Annotation.csv')
